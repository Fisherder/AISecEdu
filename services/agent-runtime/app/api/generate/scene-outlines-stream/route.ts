...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const prompts = buildPrompt(promptId, {
      ...
      // Cybersecurity subject profile + curriculum context (激活安全教学法 + 课表对齐).
      subjectProfile: requirements.subjectProfile === 'cybersecurity',
      curriculumContext: buildCurriculumContext(requirements.courseId),
    });
      ...
      async start(controller) {
        ...
        try {
          ...
          if (parsedOutlines.length > 0) {
            ...
            const fallback = buildFallbackOutlines(requirements, {
              subjectProfile: requirements.subjectProfile === 'cybersecurity',
            });
            const usedFallbackIds = new Set<string>();
            parsedOutlines = fallback.outlines.map((outline, index) => {
            ...
            log.warn(
              `Outline generation failed after ${MAX_STREAM_RETRIES + 1} attempts; emitted deterministic fallback`,
              { lastError },
            );
            const doneEvent = JSON.stringify({
              type: 'done',
              ...
              courseTitle: fallback.courseTitle,
              taskEngineMode,
              generationFallback: true,
              fallbackReason: lastError || 'empty model response',
            });
            controller.enqueue(encoder.encode(`data: ${doneEvent}\n\n`));
          }
        } catch (error) {
