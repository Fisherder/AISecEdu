...
import { ClassroomLearningTracker } from '@/components/classroom-learning-tracker';

const log = createLogger('Classroom');

async function reportPersonalStudy(type: string, payload: Record<string, unknown>) {
  await fetch('/api/integration/aisecedu/events', {
    method: 'POST',
...
export default function ClassroomDetailPage() {
  ...
  useEffect(() => {
    if (!classroomId.startsWith('study_')) return;
    void reportPersonalStudy('personal-study.opened', {
      artifactId: classroomId.slice(6),
    });
  }, [classroomId]);

  useEffect(() => {
    if (!classroomId.startsWith('study_') || loading || !currentSceneId) return;
    void reportPersonalStudy('personal-study.scene-viewed', {
      artifactId: classroomId.slice(6),
      sceneId: currentSceneId,
    });
  }, [classroomId, currentSceneId, loading]);
  ...
  return (
    <ThemeProvider>
      <MediaStageProvider value={classroomId}>
        <ClassroomLearningTracker classroomId={classroomId} active={!loading && !error} />
        <div className="h-screen flex flex-col overflow-hidden">
          {loading ? (
            ...
            <Stage onRetryOutline={retrySingleOutline} allowEditMode={false} />
          )}
        </div>
      </MediaStageProvider>
    </ThemeProvider>
