...
export interface DiagramConfig {
  ...
  diagramType:
    ...
    | 'hierarchy'
    | 'system'
    | 'attack-tree' // 攻击树：根目标→子目标，含 AND/OR 节点
    | 'network-topology' // 网络拓扑：主机/防火墙/路由器+链路+信任域
    | 'crypto-protocol' // 密码协议时序图：参与方+消息流+密钥派生
    | 'threat-model' // 威胁建模图：资产/威胁/控制（STRIDE 风格）
    | 'mitre-matrix'; // MITRE ATT&CK 矩阵：战术×技术
  description: string;
  nodes: DiagramNode[];
  edges: DiagramEdge[];
