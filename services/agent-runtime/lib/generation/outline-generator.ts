...
export function buildFallbackOutlines(
  requirements: UserRequirements,
  options: { subjectProfile?: boolean } = {},
): { languageDirective: string; courseTitle: string; outlines: SceneOutline[] } {
  const topic = (requirements.requirement || '').replace(/\s+/g, ' ').trim() || '课程主题';
  const courseTitle = topic.slice(0, 120);
  const subjectHint = options.subjectProfile
    ? '在授权、隔离的教学环境中强调威胁建模、证据链和修复验证。'
    : '先建立概念，再通过观察、练习和复盘形成可迁移的判断。';
  const keyPoints = [topic.slice(0, 180), subjectHint];

  const outlines: SceneOutline[] = [
    {
      id: nanoid(),
      type: 'slide',
      title: `${courseTitle}：核心概念`,
      description: `围绕“${topic}”建立共同语言、学习目标和判断边界。`,
      keyPoints,
      order: 1,
    },
    {
      id: nanoid(),
      type: 'interactive',
      title: `${courseTitle}：观察与演示`,
      description: `通过一个可重置的本地模拟观察“${topic}”中的状态变化，并记录证据。`,
      keyPoints: [...keyPoints, '说明观察到的状态、证据和下一步验证。'],
      widgetType: 'simulation',
      widgetOutline: { concept: topic, keyVariables: [] },
      order: 2,
    },
    {
      id: nanoid(),
      type: 'quiz',
      title: `${courseTitle}：形成性练习`,
      description: `用一个短测验检查对“${topic}”的理解，并解释选择依据。`,
      keyPoints: [...keyPoints, '用证据解释答案而不是只复述术语。'],
    ...
    },
    {
      id: nanoid(),
      type: 'slide',
      title: `${courseTitle}：复盘与迁移`,
      description: `整理“${topic}”的关键结论、边界条件和可迁移行动。`,
      keyPoints: [...keyPoints, '写出一个可以在新情境中验证的行动。'],
      order: 4,
    },
  ];

  return { languageDirective: DEFAULT_LANGUAGE_DIRECTIVE, courseTitle, outlines };
}
...
export async function generateSceneOutlinesFromRequirements(
  ...
  options?: {
    ...
    /** 网安学科画像开启时为 true，触发 prompt 中的 {{#if subjectProfile}} 片段。 */
    subjectProfile?: boolean;
    /** 由 buildCurriculumContext 构造的课程上下文文本（unknown/空则不注入）。 */
...
> {
  ...
  const prompts = buildPrompt(PROMPT_IDS.REQUIREMENTS_TO_OUTLINES, {
    ...
    // Cybersecurity subject profile + curriculum context (Plan 1 plumbing;
    // the prompt templates consume these in a later plan).
    subjectProfile: options?.subjectProfile ?? false,
    curriculumContext: options?.curriculumContext || 'None',
  });
