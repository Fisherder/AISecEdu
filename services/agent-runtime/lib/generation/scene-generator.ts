...
import { generateSlideViaWorkflow } from './slide-workflow';
import {
...
export interface SceneContentOptions {
  ...
  /** 网安学科画像开启时为 true，触发场景内容 prompt 的 {{#if subjectProfile}} 片段。 */
  subjectProfile?: boolean;
  /** 显式控制是否走 widget 工作流（解耦于 subjectProfile；false=原版定制 prompt）。 */
  useWorkflow?: boolean;
  /** 安全知识库 RAG 上下文（注入到 prompt 作为权威事实参考）。 */
...
> {
  ...
  if (outline.type === 'interactive') {
    ...
    return generateWidgetContent(outline, aiCall, languageDirective, { allowProceduralSkill, subjectProfile, useWorkflow, securityKnowledge });
  }

  switch (outline.type) {
    case 'slide':
      // Workflow (fast) vs original bespoke slide prompt. Defaults to
      // subjectProfile (MVP behavior); rich mode passes useWorkflow=false.
      if (useWorkflow ?? subjectProfile) {
        return generateSlideViaWorkflow(outline, aiCall, { languageDirective, subjectProfile: true });
      }
    ...
    case 'quiz':
      return generateQuizContent(outline, aiCall, languageDirective, subjectProfile, securityKnowledge);
    case 'pbl':
...
export async function generateWidgetContent(
  ...
  aiCall: AICallFn,
  languageDirective?: string,
  options: { allowProceduralSkill?: boolean; subjectProfile?: boolean; useWorkflow?: boolean; securityKnowledge?: string } = {},
  ...
  // Workflow vs original bespoke generation. Defaults to subjectProfile (MVP
  // behavior); the prep workspace passes useWorkflow explicitly so "精细"
  // (rich) mode can use the original prompts even under the security profile.
  const useWf = options.useWorkflow ?? options.subjectProfile ?? false;

  // Cybersecurity profile: use the small-call workflow for diagram widgets so
  // weaker models (e.g. DeepSeek Flash) don't hang on the giant single call.
  if (widgetType === 'diagram' && useWf) {
    return generateDiagramViaWorkflow(outline, aiCall, { languageDirective, subjectProfile: true });
  }
  if (widgetType === 'code' && useWf) {
    return generateCodeViaWorkflow(outline, aiCall, { languageDirective, subjectProfile: true });
  }
  if (widgetType === 'procedural-skill' && useWf) {
    return generateProceduralSkillViaWorkflow(outline, aiCall, { languageDirective, subjectProfile: true });
  }
  if (widgetType === 'simulation' && useWf) {
    return generateSimulationViaWorkflow(outline, aiCall, { languageDirective, subjectProfile: true });
  }
  if (widgetType === 'game' && useWf) {
    return generateGameViaWorkflow(outline, aiCall, { languageDirective, subjectProfile: true });
  }
  if (widgetType === 'visualization3d' && useWf) {
    return generateViz3DViaWorkflow(outline, aiCall, { languageDirective, subjectProfile: true });
  }
  ...
  // Inject the security subject profile so {{#if subjectProfile}} blocks in
  // code / procedural-skill / diagram / simulation / game / 3D templates render.
  variables = { ...variables, subjectProfile: options.subjectProfile, securityKnowledge: options.securityKnowledge };
...
export async function generateSceneActions(
  ...
  if (outline.type === 'slide' && 'elements' in content) {
    ...
    try {
      ...
      log.warn(`slide-actions generation fell back for: ${outline.title}`, error);
      return generateDefaultSlideActions(outline, content.elements);
    }
  ...
  if (outline.type === 'quiz' && 'questions' in content) {
    ...
    try {
      ...
      log.warn(`quiz-actions generation fell back for: ${outline.title}`, error);
      return generateDefaultQuizActions(outline);
    }
    const actions = parseActionsFromStructuredOutput(response, outline.type);
  ...
  if (outline.type === 'interactive' && 'html' in content) {
    ...
    try {
      ...
      log.warn(`interactive-actions generation fell back for: ${outline.title}`, error);
      return generateDefaultInteractiveActions(outline);
    }
  ...
  if (outline.type === 'pbl' && 'projectConfig' in content) {
    ...
    try {
      ...
      log.warn(`pbl-actions generation fell back for: ${outline.title}`, error);
      return generateDefaultPBLActions(outline);
    }
    const actions = parseActionsFromStructuredOutput(response, outline.type);
