...
/** Base directory for usage logs; lands in the agent-runtime data volume in Docker. */
...
// These calls run inside the service-authenticated 玄甲 job endpoint, not
// inside a standalone browser session. Their usage is merged into the job
// response and persisted once by ``dojo_plugin.agent_runtime.jobs`` together
// with the durable job/model audit row. Forwarding the same usage through the
// session endpoint would both double-count it and resurrect a runtime-specific
// session dependency in the unified global Agent.
const JOB_REPORTED_USAGE_SOURCES = new Set([
  'aisecedu-agent-plan',
...
export function isJobReportedUsageSource(source: string): boolean {
  return JOB_REPORTED_USAGE_SOURCES.has(source);
}

/**
 * Records one generation's usage without propagating logging failures.
 *
...
export async function recordUsage(
  ...
  try {
    ...
    if (isAISecEduIntegrated()) {
      if (isJobReportedUsageSource(input.source)) return;
      try {
        await aiseceduRequest('/pwncollege_api/v1/teaching/runtime/usage', {
          method: 'POST',
          body: JSON.stringify(record),
        });
      } catch (error) {
        // Classroom media may be rendered as part of a service-authenticated
        // durable job. Those calls have no browser session to which a
        // standalone modality event can be attributed; their primary model
        // usage is already retained on the durable job invocation. Treat this
        // specific, expected condition as a no-op rather than issuing a noisy
        // warning for every generated page.
        if (
          error instanceof Error &&
          error.message.includes('玄甲 global-agent session is missing')
        ) {
          return;
        }
        throw error;
 ...
 * Resolves after the usage record has been accepted or safely ignored.
 */
...
export async function readUsageRecords(opts: ReadOptions = {}): Promise<UsageRecord[]> {
  if (isAISecEduIntegrated() && !opts.baseDir) return [];
  const dir = usageDir(opts.baseDir);
  let files: string[];
  try {
