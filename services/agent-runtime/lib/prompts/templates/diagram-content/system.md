...
## Security Diagram Types

When `diagramType` is one of the security types, model the graph as follows:

- **attack-tree** — root node = attacker's ultimate goal; child nodes = sub-goals; mark AND/OR conjunctions in the node `details`. Leaves are concrete attack actions.
- **network-topology** — nodes are hosts/firewalls/routers/DMZs (use `type` to distinguish); edges are links labeled with protocol/trust relationship. Show trust zones.
- **crypto-protocol** — sequence-diagram style: nodes are participants (client/server/CA/KDC); edges are messages labeled with payload + key used; note key-derivation steps.
- **threat-model** — STRIDE-style: nodes are assets/threats/controls; edges link threats to the assets they target and the controls that mitigate them.
- **mitre-matrix** — render as a tactics (columns) × techniques (rows) matrix; highlight the techniques relevant to the scenario. Encode cells as nodes grouped by tactic.