...
export function isPiWebSearchEnabled(): boolean {
  return readBoolean(
    process.env.GLOBAL_AGENT_RUNTIME_ENABLE_PI_WEB_SEARCH ??
      process.env.OPENMAIC_ENABLE_PI_WEB_SEARCH,
  );
}
...
export function resolveVocationalActive(
  ...
  // Cybersecurity subject profile opts into procedural-skill scenes
  // (pen-test flow, incident response, reverse-engineering steps, forensics)
  // without requiring the vocational task-engine env flag.
  if (requirements?.subjectProfile === 'cybersecurity') return true;
