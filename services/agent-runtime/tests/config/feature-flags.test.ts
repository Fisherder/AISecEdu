...
describe('isPiWebSearchEnabled', () => {
  const flag = 'GLOBAL_AGENT_RUNTIME_ENABLE_PI_WEB_SEARCH';
  const legacyFlag = 'OPENMAIC_ENABLE_PI_WEB_SEARCH';
  let original: string | undefined;
  let originalLegacy: string | undefined;

  beforeEach(() => {
    original = process.env[flag];
    originalLegacy = process.env[legacyFlag];
    delete process.env[flag];
  ...
  afterEach(() => {
    ...
    if (originalLegacy === undefined) delete process.env[legacyFlag];
    else process.env[legacyFlag] = originalLegacy;
  });
  ...
  it('reads the old name only as a migration fallback', () => {
