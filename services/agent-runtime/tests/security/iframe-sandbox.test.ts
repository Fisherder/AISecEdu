...
function hasEscapeCapability(sandbox: string): boolean {
  const tokens = sandbox.split(/\s+/);
  return tokens.includes('allow-popups') || tokens.includes('allow-top-navigation');
}
...
describe('iframe sandbox safety', () => {
  test('InteractiveIframeHost does not combine allow-scripts + allow-same-origin', () => {
    ...
    for (const sandbox of sandboxes) {
      ...
      expect(hasEscapeCapability(sandbox)).toBe(false);
    }
  });
  ...
  test('integrated artifact preview cannot open or navigate parent windows', () => {
    const sandboxes = extractSandboxValues('app/prep/[id]/page.tsx');
    expect(sandboxes.length).toBeGreaterThan(0);
    for (const sandbox of sandboxes) {
      expect(isDangerousSandbox(sandbox)).toBe(false);
      expect(hasEscapeCapability(sandbox)).toBe(false);
