...
describe('LLM thinking provider options', () => {
  ...
  it('retries a reasoning-only response with thinking disabled', async () => {
    const thinkingModes: Array<unknown> = [];
    const currentThinking = () =>
      (
        globalThis as unknown as {
          __thinkingContext?: { getStore?: () => unknown };
        }
      ).__thinkingContext?.getStore?.();
    const noOutput = Object.assign(new Error('No output generated.'), {
      name: 'AI_NoOutputGeneratedError',
    });
    aiMock.generateText
      .mockImplementationOnce(async () => {
        thinkingModes.push(currentThinking());
        throw noOutput;
      })
      .mockImplementationOnce(async (params: unknown) => {
        thinkingModes.push(currentThinking());
        return { text: 'ok', params };
      });
    ...
    const result = await callLLM(
      {
        ...
        prompt: 'return a compact JSON edit',
      } as Parameters<typeof callLLM>[0],
      'test',
      { retries: 1 },
    ...
    expect(thinkingModes).toEqual([
      { mode: 'enabled', effort: 'high' },
      { mode: 'disabled', enabled: false, effort: 'none' },
    ]);
  });
