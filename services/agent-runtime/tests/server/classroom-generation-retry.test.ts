...
vi.mock('@/lib/generation/outline-generator', () => ({
  ...
  buildFallbackOutlines: mocks.buildFallbackOutlines,
...
describe('classroom scene generation retries', () => {
  beforeEach(() => {
    ...
    mocks.buildFallbackOutlines.mockReturnValue({
      languageDirective: 'Use English.',
  ...
  it('keeps the classroom usable when outline and scene content model calls fail', async () => {
    mocks.generateSceneOutlinesFromRequirements.mockResolvedValue({
      success: false,
      error: 'provider unavailable',
    });
    mocks.generateSceneContent.mockRejectedValue(new Error('provider unavailable'));

    const { result } = await generateWithProgress();

    expect(result.scenesCount).toBe(1);
    expect(mocks.buildFallbackOutlines).toHaveBeenCalledTimes(1);
    expect(mocks.generateSceneContent).toHaveBeenCalledTimes(1);
  });

  it('keeps the classroom usable when the classroom model cannot be resolved', async () => {
    mocks.resolveModel.mockRejectedValue(new Error('DEFAULT_MODEL is not configured'));
    mocks.generateSceneOutlinesFromRequirements.mockRejectedValue(new Error('model unavailable'));
    mocks.generateSceneContent.mockRejectedValue(new Error('model unavailable'));
