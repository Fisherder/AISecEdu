...
describe('recordUsage — LLM', () => {
  it('leaves service-job usage to the durable 玄甲 job audit row', () => {
    expect(isJobReportedUsageSource('aisecedu-agent-plan')).toBe(true);
    expect(isJobReportedUsageSource('aisecedu-integration-job')).toBe(true);
    expect(isJobReportedUsageSource('aisecedu-agent-deliverable-closure')).toBe(true);
    expect(isJobReportedUsageSource('aisecedu-material-segment-analysis')).toBe(true);
    expect(isJobReportedUsageSource('aisecedu-material-analysis-synthesis')).toBe(true);
    expect(isJobReportedUsageSource('generate-classroom')).toBe(true);
    expect(isJobReportedUsageSource('revise-simulation')).toBe(true);
    expect(isJobReportedUsageSource('scene-content')).toBe(false);
  });
